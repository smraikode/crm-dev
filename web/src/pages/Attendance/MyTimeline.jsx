...
const MyTimeline = () => {
  ...
  const fetchSiteLocation = async () => {
    ...
    try {
      ...
      console.log("📍 latitude:", office.latitude);
      console.log("📍 longitude:", office.longitude);
  ...
  // 🔐 Validate + parallel fetch on mount
  useEffect(() => {
    const isValid = checkUserValid();
    if (!isValid) {
      navigate("/login");
    ...
    // Prompt for geolocation permission early
    navigator.geolocation.getCurrentPosition(() => {}, () => {});
    fetchSiteLocation();
    fetchAndSetLocation(); // Parallel fetch
  }, [navigate]);
  ...
  const handleClockIn = async () => {
    ...
    if (distance !== null && distance <= 1500) {
      ...
      alert(`❌ You are ${distance} meters away from the site. Must be within 1500m.`);
    }
  };
