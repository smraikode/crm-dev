    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (!currentEmail) return;
          ...
          `${apiEndpoints.getSubordinates}/${currentEmail}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );

        const subordinateRaw = subRes.data.subordinates || [];

        // Step 2: Extract emails
        const subordinateEmails = subordinateRaw
          .map((item) => item.split(" - ")[1]?.trim())
          .filter(Boolean);

        setEmailList(subordinateEmails);

        // Step 3: Fetch all attendance data
        const attRes = await axios.get(apiEndpoints.MyTeamAttendance, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });

        const allData = attRes.data.data || [];

        // Step 4: Filter data for subordinates only
        const subData = allData.filter((item) =>
          subordinateEmails.includes(item.email)
        );

        setAttendanceData(subData);
      } catch (error) {
